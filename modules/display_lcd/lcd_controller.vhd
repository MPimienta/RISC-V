...
architecture Behavioral of lcd_controller is
    ...
    constant MS_50   : integer := 5_000;
    constant MS_5    : integer := 500;
    constant US_200  : integer := 20;
    constant US_50   : integer := 5;
    ...
    signal next_i2c_state : state_type := IDLE; -- Nueva señal
                ...
                case state is
                    ...
                    when NIBBLE_HIGH_EN1 =>
                        if i2c_busy = '0' and s_i2c_ena = '0' then
                            ...
                            s_i2c_ena <= '1';
                            next_i2c_state <= NIBBLE_HIGH_EN0; -- Guardamos el siguiente paso
                            state <= WAIT_I2C;
                        end if;
                    ...
                    when NIBBLE_HIGH_EN0 =>
                        if i2c_busy = '0' and s_i2c_ena = '0' then
                            ...
                            s_i2c_ena <= '1';
                            next_i2c_state <= NIBBLE_LOW_EN1; -- Guardamos el siguiente paso
                            state <= WAIT_I2C;
                        end if;
                    ...
                    when NIBBLE_LOW_EN1 =>
                        if i2c_busy = '0' and s_i2c_ena = '0' then
                            ...
                            s_i2c_ena <= '1';
                            next_i2c_state <= NIBBLE_LOW_EN0; -- Guardamos el siguiente paso
                            state <= WAIT_I2C;
                        end if;
                    ...
                    when NIBBLE_LOW_EN0 =>
                        if i2c_busy = '0' and s_i2c_ena = '0' then
                            ...
                            s_i2c_ena <= '1';
                            next_i2c_state <= return_state; -- Fin del byte, volvemos a IDLE o INIT
                            state <= WAIT_I2C;
                        end if;

                    -- Sincronización I2C simplificada e infalible
                    when WAIT_I2C =>
                        if i2c_busy = '1' then
                            s_i2c_ena <= '0'; 
                        elsif i2c_busy = '0' and s_i2c_ena = '0' then
                            state <= next_i2c_state; -- Saltamos sin preguntar
                        end if;
